 ...
 *
 * File: admin/roles/edit.php
 * Purpose: Form to edit an existing role
 */
...
$formData = [
    ...
    'can_raise_requisition' => $role['can_raise_requisition'],
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!Session::validateCsrfToken($_POST['csrf_token'] ?? '')) {
        ...
        // Check if role name or code already exists (excluding current role)
        if (empty($errors)) {
            $existingRole = $db->fetchOne("SELECT id FROM roles WHERE (role_name = ? OR role_code = ?) AND id != ?", [
                $formData['role_name'],
                $formData['role_code'],
                $roleId
            ]);
        ...
        if (empty($errors)) {
            try {
                ...
                if ($result) {
                    Session::setFlash('success', 'Role "' . $formData['role_name'] . '" updated successfully.');
                    redirect('list.php');
                } else {
...
<div class="page-header">
    <h1><i class="fas fa-user-shield"></i> Edit Role</h1>
...
<div class="form-container">
    <?php if ($userCount['count'] > 0): ?>
        <div class="info-banner">
            <i class="fas fa-info-circle"></i>
            <div>
                <strong>Note:</strong> This role is currently assigned to <?php echo $userCount['count']; ?> active user(s).
